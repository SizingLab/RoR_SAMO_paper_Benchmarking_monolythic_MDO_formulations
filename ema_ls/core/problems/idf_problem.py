...
from ema.core.models.idf_models import Actuator, ActuatorBlackBox
...
class IDFProblem(BenchmarkProblem):
    ...
    def initialize_problem(self):
        ...
        # * 2 for inertial load contribution to T_em
        model.add_design_var('T_em', lower=0.1, upper=100. * self.scale * 2.)
        model.add_design_var('J_mot', lower=1.e-6, upper=1.e-1 * self.scale ** (5.0 / 3.5))

        # Adding constraints
        if self.prob_type == 'MDO':
            model.add_constraint('W_mot_constr', upper=0.)
        model.add_constraint('T_em_c_constr', lower=0., upper=0.)
    ...
    def check_success(self):
        ...
        relative_error = abs(self.problem['T_em_c_constr']) / self.problem['T_em']
        ...
        s = 'Success in solving system consistency: ' + str(success) + '\n' \
            ...
            self.problem['N_red'][0]) + '\n' \
                                        'Motor torque consistency constraint: ' + str(
            self.problem['T_em_c_constr'][0]) + '\n' \
