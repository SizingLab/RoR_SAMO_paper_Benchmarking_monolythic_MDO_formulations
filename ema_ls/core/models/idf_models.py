...
class MotorTorqueConsistencyConstraint(ExplicitComponent):
    """ExplicitComponent that computes the motor torque consistency constraint for IDF."""
    ...
    def setup(self):
        ...
        # Motor torque consistency design variable
        self.add_input('T_em_t', val=1.)

        # Motor torque
        self.add_input('T_em', val=1.)

        # Motor torque consistency constraint
        self.add_output('T_em_c_constr', val=1.)
    ...
    def compute(self, inputs, outputs):
        ...
        T_em_c_constr = T_em_t - T_em"""

        T_em_t = inputs['T_em_t']
        T_em = inputs['T_em']

        outputs['T_em_c_constr'] = T_em_t - T_em

    def compute_partials(self, inputs, partials):
        """ Jacobian for motor torque consistency constraint."""

        partials['T_em_c_constr', 'T_em_t'] = 1.
        partials['T_em_c_constr', 'T_em'] = -1.
...
class Actuator(Group):
    ...
    def setup(self):
        ...
        self.add_subsystem('pT_em', IndepVarComp('T_em', 1.), promotes=['T_em'])
        ...
        self.add_subsystem('motor_torque_c_constr',
                           MotorTorqueConsistencyConstraint(derivative_method=self.derivative_method),
                           promotes=['T_em_c_constr'])

        # Connections
        self.connect('T_em', 'motor_torque_c_constr.T_em_t')
        self.connect('motor_torque.T_em', 'motor_torque_c_constr.T_em')
        self.connect('T_em', 'motor_speed.T_em')
        self.connect('T_em', 'motor_mass.T_em')
...
class ActuatorBlackBoxComponent(ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        outputs['T_em_c_constr'] = p['T_em_c_constr']
    ...
    def setup(self):
        ...
        self.add_subsystem('actuator_blackbox',
                           ...
                           promotes=['T_em', 'J_mot', 'a_n', 'b_n', 'A_ema', 'N_red', 'p', 'F_ema', 'T_em_ref',
                                     ...
                                     'M_mot', 'W_mot_constr', 'T_em_c_constr', 'J_mot_c_constr', 'X_final', 'V_final'])
